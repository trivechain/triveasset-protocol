...
var triveassetbuilder = new TriveAssetBuilder({network: 'testnet'})
...
describe('builder.buildIssueTransaction(args)', function () {
  it('throws: Must have "utxos"', function (done) {
    ...
    assert.throws(function () {
      triveassetbuilder.buildIssueTransaction(args)
    }, /Must have "utxos"/)
    done()
  })
  ...
  it('returns valid response with default values', function (done) {
    var result = triveassetbuilder.buildIssueTransaction(issueArgs)
    assert(result.txHex)
  ...
  it('flags.injectPreviousOutput === true: return previous output hex in inputs', function (done) {
    ...
    var result = triveassetbuilder.buildIssueTransaction(args)
    assert(result.txHex)
    var tx = Transaction.fromHex(result.txHex)
    assert.equal(tx.ins.length, 1)
...
describe('builder.buildSendTransaction(args)', function () {
  it('throws: Must have "utxos"', function (done) {
    ...
    delete args.utxos
    assert.throws(function () {
      triveassetbuilder.buildSendTransaction(args)
    }, /Must have "utxos"/)
    done()
  })
  ...
  it('returns valid response with default values', function (done) {
    var result = triveassetbuilder.buildSendTransaction(sendArgs)
    assert(result.txHex)
    var tx = Transaction.fromHex(result.txHex)
    assert.equal(tx.ins.length, 1)
  ...
  it('returns valid response with default values', function (done) {
    ...
    args.sha2 = '59040d5c3bc91b5e28e014541363c0f64d9a2429541fe6cf1c568c63c85fbb20'
    args.torrentHash = '02fcc3d843eaba4d278ed107c0c2b56a146f66b8'
    var result = triveassetbuilder.buildSendTransaction(args)
    assert(result.txHex)
    var tx = Transaction.fromHex(result.txHex)
...
describe('builder.buildBurnTransaction(args)', function () {
  it('returns valid response with default values', function (done) {
    var result = triveassetbuilder.buildBurnTransaction(burnArgs)
    assert(result.txHex)
    var tx = Transaction.fromHex(result.txHex)
    assert.equal(tx.ins.length, 1)
    assert.equal(tx.outs.length, 3) // OP_RETURN + 2 changes
